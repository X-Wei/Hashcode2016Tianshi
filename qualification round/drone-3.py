...
estimated_ft = {} # mapping order id to estimated finish time
...
def update_est_ft():
    ...
    for o in xrange(C):
        ...
        estimated_ft[o] = estimate_finishtime(od, min_finishtime)
        min_finishtime = min(estimated_ft[o], min_finishtime)

def next_order(cnt):
    res, min_finishtime =  None, 999999999
    if cnt%3==0: 
        update_est_ft() # update the estimation every 100 turns
    min_finishtime = 999999999
    for o in estimated_ft:
        od = all_orders[o]
        if od.finished()==True: 
            #~ del estimated_ft[o]
            continue
...
for _ in xrange(C):
    cmds = []
    od, ft = next_order(_) # get the easiest order
    if ft == 999999999:
        update_est_ft()
        od, ft = next_order(_) 
    print od.id, ft
